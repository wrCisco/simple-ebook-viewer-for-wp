...
const fragmentToSSML = async (fragment, inherited, SRE) => {
    const nodeName = fragment.firstChild.nodeName.toLowerCase()
    const ssml = document.implementation.createDocument(NS.SSML, 'speak')
    const { lang } = inherited
    await SRE.updateLocale(lang)
...
const getFragmentWithMarks = async (range, textWalker, granularity, SRE) => {
    ...
    const ssml = await fragmentToSSML(fragment, { lang, alphabet }, SRE)
    return { entries, ssml }
}
...
export class TTS {
    ...
    constructor(doc, textWalker, highlight, granularity, SRE) {
        ...
        this.#list = new ListIterator(getBlocks(doc), async range => {
            const { entries, ssml } = await getFragmentWithMarks(range, textWalker, granularity, SRE)
    ...
    async resume() {
        const [doc] = (await this.#list.current()) ?? []
        if (!doc) return await this.next(true)
        return this.#speak(doc, ssml => this.#getMarkElement(ssml, this.#lastMark))
    }
    async prev(paused) {
        this.#lastMark = null
        const [doc, range] = (await this.#list.prev()) ?? []
        if (paused && range) this.highlight(range.cloneRange())
        return this.#speak(doc)
    }
    async next(paused) {
        this.#lastMark = null
        const [doc, range] = (await this.#list.next()) ?? []
        if (paused && range) this.highlight(range.cloneRange())
        return this.#speak(doc)
    }
    async from(range) {
